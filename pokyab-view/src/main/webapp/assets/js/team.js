...
//                {data: 'trainerPerName'},
    ...
    //teamPersons.jsp table
    $table = $('#teamPersonsListTable');

    if ($table.length) {

        var jsonUrl = '';
        jsonUrl = window.contextRoot + '/json/data/admin/all/' + window.teamId + '/teamPersons';

        console.log('Inside js teamPersonsListTable');
        console.log('jsonUrl: ' + jsonUrl);
        $table.DataTable({
            "language": {
                "loadingRecords": "Cargando...",
            ...
            columns: [
                {
                    ...
                    mRender: function (data, type, row) {
                        var str = '';
                        str += '<a href="'+ window.contextRoot + '/manage/teamPerson/'
                                + data + '"><i class="fas fa-edit"></i></a>';
                        return str;
                    }
                },
                ...
                {data: 'startDate'},
                {
                    ...
                    mRender: function (data, type, row) {
                        ...
                        if (data == "Y") {
                            str += '<label class="switch">'
                                    + '<input type="checkbox" checked="checked" value="' + row.teamPersonId + '"/>'
                                    + '<div class="slider round"></div></label>';
                        } else {
                            str += '<label class="switch">'
                                    + '<input type="checkbox" value="' + row.teamPersonId + '"/>'
                                    + '<div class="slider round"></div></label>';
                        }
                        return str;
                    }
                    ...
                    bootbox.confirm({
                        ...
                        callback: function (confirmed) {
                            if (confirmed) {
                                console.log(value);
                                //Post update product
                                var activationUrl = window.contextRoot + '/manage/teamPerson/' + value + '/activation';
                                console.log('activationUrl: ' + activationUrl);
                                $.ajax({
                                    type: 'GET',
                                    url: activationUrl,
